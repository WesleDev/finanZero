...
const TrendingUpIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    className={className || 'h-6 w-6 mr-2'}
    fill='none'
    viewBox='0 0 24 24'
    stroke='currentColor'
  >
...
const CurrencyDollarIcon: React.FC<{ className?: string }> = ({
  className,
}) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
  ...
  >
    <path
      strokeLinecap='round'
      strokeLinejoin='round'
      d='M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01M12 6v-1m0-1V4m0 2.01M12 18v-1m0-1v-1m0 0v-1m0 0V9.99M12 18h.01M12 21a9 9 0 110-18 9 9 0 010 18z'
    />
  </svg>
);
...
const calculateMonthlyExpensesForDate = (
  expenses: Expense[],
  date: Date
): number => {
  ...
  return expenses.reduce((total, expense) => {
    if (expense.isRecurring) {
      ...
      if (
        ...
        effectiveStartDate.getDate() >= CARD_CLOSING_DAY
      ) {
      ...
      if (
        expense.category === 'Cartão de Crédito' &&
        firstPaymentDate.getDate() >= CARD_CLOSING_DAY
      ) {
      ...
      if (
        expense.category === 'Cartão de Crédito' &&
        effectiveDate.getDate() >= CARD_CLOSING_DAY
      ) {
    ...
    >
      ...
      >
        ...
        <div className='p-4 bg-dark-700/50 flex justify-end gap-4 rounded-b-lg'>
          <button
            onClick={onClose}
            className='bg-dark-600 hover:bg-dark-500 text-slate-200 font-bold py-2 px-4 rounded-lg transition-colors'
          >
            Cancelar
          </button>
          <button
...
export default function App() {
  ...
  const [fortnightlyIncome, setFortnightlyIncome] = useLocalStorage<number>(
    'fortnightlyIncome',
    0
  );
  const [monthlyPayment, setMonthlyPayment] = useLocalStorage<number>(
    'monthlyPayment',
    0
  );
  const [midMonthPercentages, setMidMonthPercentages] = useLocalStorage<
    Record<string, number>
  >('midMonthPercentages', {});
  const [endOfMonthPercentages, setEndOfMonthPercentages] = useLocalStorage<
    Record<string, number>
  >('endOfMonthPercentages', {});
  const [isCensored, setIsCensored] = useLocalStorage<boolean>(
    'isCensored',
    false
  );
  const [investmentFrequency, setInvestmentFrequency] = useLocalStorage<
    'bi-monthly' | 'monthly'
  ...
  const { displayedMonthExpenses, previousMonthExpenses, surplus } =
    useMemo(() => {
    ...
    setConfirmation({
      ...
      onConfirm: () =>
        setIncomes((currentIncomes) =>
          currentIncomes.filter((i) => i.id !== id)
        ),
    });
  };
  ...
  const addJar = (jar: Omit<SavingsJar, 'id'>) => {
    ...
    setMidMonthPercentages((prev) => ({
      ...prev,
      [newJarWithId.id]: newJarWithId.percentage,
    }));
    setEndOfMonthPercentages((prev) => ({
      ...prev,
      [newJarWithId.id]: newJarWithId.percentage,
    }));
  ...
  const updateJarPercentage = (id: string, percentage: number) => {
    const newJars = jars.map((jar) =>
      jar.id === id ? { ...jar, percentage } : jar
    );
    const totalPercentage = newJars.reduce(
      (acc, jar) => acc + jar.percentage,
      0
    );
    ...
    reader.onload = (e) => {
      try {
        ...
        if (
          ...
          !data.expenses ||
          !data.jars ||
          data.fortnightlyIncome === undefined
        ) {
        ...
        if (
          window.confirm(
            'Tem certeza que deseja importar os dados? Todos os dados atuais serão substituídos.'
          )
        ) {
      ...
      <main className='container mx-auto p-4 md:p-8'>
        ...
        {activeTab === 'dashboard' && (
          <div id='dashboard-content'>
            <MonthlyAlert
              current={displayedMonthExpenses}
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6'>
              <div className='bg-dark-800 p-6 rounded-xl shadow-lg'>
                ...
                <p className='text-green-400 text-2xl sm:text-3xl font-bold'>
                  {formatCurrency(totalIncome, isCensored)}
                </p>
              </div>
              <div className='bg-dark-800 p-6 rounded-xl shadow-lg'>
                <h2 className='text-slate-400 text-lg'>Gastos do Mês</h2>
                <p className='text-red-400 text-2xl sm:text-3xl font-bold'>
                  {formatCurrency(displayedMonthExpenses, isCensored)}
                </p>
              </div>
              <div className='bg-dark-800 p-6 rounded-xl shadow-lg'>
                ...
                <p
                  className={`${
                    surplus >= 0 ? 'text-blue-400' : 'text-yellow-400'
...
const TabNavigation: React.FC<{
  ...
  return (
    <div className='mb-8 border-b border-dark-700'>
      <nav
        className='-mb-px flex justify-center sm:justify-start space-x-4 sm:space-x-6'
        aria-label='Tabs'
      >
        {tabs.map((tab) => (
          <button
  ...
  if (isCensored) {
    return (
      <div className='text-center py-10 text-slate-400'>
        Desative o modo de ocultar valores para ver o gráfico.
      </div>
    );
  }
      ...
      >
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          ...
          {data.map((d, i) => (
            <g
              key={d.month}
              transform={`translate(${xScale(i)}, ${innerHeight})`}
            >
              <text
                y='20'
                textAnchor='middle'
          ...
          {/* Lines */}
          <path
            d={lineGenerator('income')}
            fill='none'
            stroke={colors.income}
            strokeWidth='3'
          />
          <path
            d={lineGenerator('expenses')}
            fill='none'
            stroke={colors.expenses}
            strokeWidth='3'
          />
          <path
            d={lineGenerator('surplus')}
            fill='none'
          ...
          {tooltip && (
            <g>
              ...
              <circle
                cx={tooltip.x}
                cy={yScale(tooltip.dataPoint.income)}
                r='5'
                fill={colors.income}
              ...
              <circle
                cx={tooltip.x}
                cy={yScale(tooltip.dataPoint.expenses)}
                r='5'
              ...
              <circle
                cx={tooltip.x}
                cy={yScale(tooltip.dataPoint.surplus)}
                r='5'
                fill={colors.surplus}
                className='stroke-dark-800'
                strokeWidth='2'
      ...
      {tooltip && (
        <div
          ...
          style={{
            ...
            transform: `translate(${
              tooltip.x > innerWidth / 2 ? '-110%' : '10%'
            }, -50%)`,
            minWidth: '160px',
          }}
        >
          <p className='font-bold text-center text-slate-200 mb-2 capitalize'>
            {tooltip.dataPoint.month}
          </p>
          <ul className='space-y-1 text-sm'>
            <li className='flex justify-between items-center'>
              ...
              <span className='font-bold' style={{ color: colors.income }}>
                {formatCurrency(tooltip.dataPoint.income, false)}
              </span>
            </li>
            <li className='flex justify-between items-center'>
              <span className='flex items-center'>
              ...
              <span className='font-bold' style={{ color: colors.expenses }}>
                {formatCurrency(tooltip.dataPoint.expenses, false)}
              </span>
            </li>
            <li className='flex justify-between items-center'>
              <span className='flex items-center'>
                <div
                  className='w-2 h-2 rounded-full mr-2'
                  style={{ backgroundColor: colors.surplus }}
                ></div>
                Sobra:
              </span>
              <span className='font-bold' style={{ color: colors.surplus }}>
                {formatCurrency(tooltip.dataPoint.surplus, false)}
              </span>
            </li>
          </ul>
        </div>
...
const FinancialEvolutionPage: React.FC<FinancialEvolutionPageProps> = ({
  incomes,
  expenses,
  isCensored,
}) => {
    ...
    <div className='bg-dark-800 p-6 rounded-xl shadow-lg mt-2'>
      ...
      {historicalData.length > 0 ? (
        ...
        <p className='text-slate-400 text-center py-10'>
          Adicione receitas e despesas para ver sua evolução financeira.
        </p>
      )}
    </div>
  );
...
const InvestmentReport: React.FC<InvestmentReportProps> = ({
  ...
  const investmentTotals = useMemo(() => {
    const totalsByJar = jars.map((jar) => {
      const midMonthAmount =
        (midMonthSurplus > 0 ? midMonthSurplus : 0) *
        ((midMonthPercentages[jar.id] || 0) / 100);
      const endOfMonthAmount =
        (monthlyPayment > 0 ? monthlyPayment : 0) *
        ((endOfMonthPercentages[jar.id] || 0) / 100);
  ...
  return (
    <div className='bg-dark-800 p-6 rounded-xl shadow-lg'>
      <h2 className='text-xl sm:text-2xl font-bold text-slate-100 mb-6 text-center'>
        Relatório de Investimentos do Mês
      </h2>

      <ul className='space-y-3 mb-6'>
        {investmentTotals.totalsByJar
          .filter((j) => j.total > 0)
          .map((jarTotal) => (
            <li
              key={jarTotal.id}
              className='bg-dark-700 p-3 rounded-lg flex justify-between items-center transition-all hover:bg-dark-600'
            >
              <span className='font-semibold text-slate-300'>
                {jarTotal.name}
              </span>
              <span className='font-bold text-lg text-accent'>
                {formatCurrency(jarTotal.total, isCensored)}
              </span>
            </li>
          ))}
      </ul>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center text-lg sm:text-xl mt-4'>
        ...
        <span className='font-extrabold text-xl sm:text-2xl text-green-400'>
          {formatCurrency(investmentTotals.grandTotal, isCensored)}
        </span>
      </div>
    </div>
  );
...
interface InvestmentsPageProps {
  ...
  midMonthPercentages: Record<string, number>;
  setMidMonthPercentages: React.Dispatch<
    React.SetStateAction<Record<string, number>>
  >;
  endOfMonthPercentages: Record<string, number>;
  setEndOfMonthPercentages: React.Dispatch<
    React.SetStateAction<Record<string, number>>
  >;
  isCensored: boolean;
...
const InvestmentsPage: React.FC<InvestmentsPageProps> = ({
  ...
  expensesForMonth,
  fortnightlyIncome,
  setFortnightlyIncome,
  monthlyPayment,
  setMonthlyPayment,
  midMonthPercentages,
  setMidMonthPercentages,
  endOfMonthPercentages,
  setEndOfMonthPercentages,
  isCensored,
  investmentFrequency,
  setInvestmentFrequency,
}) => {
  const [fortnightlyIncomeInput, setFortnightlyIncomeInput] = useState(
    fortnightlyIncome > 0 ? fortnightlyIncome.toString().replace('.', ',') : ''
  );
  const [monthlyPaymentInput, setMonthlyPaymentInput] = useState(
    monthlyPayment > 0 ? monthlyPayment.toString().replace('.', ',') : ''
  );

  useEffect(() => {
    setFortnightlyIncomeInput(
      fortnightlyIncome > 0
        ? fortnightlyIncome.toString().replace('.', ',')
        : ''
    );
  }, [fortnightlyIncome]);
  ...
  const creditCardExpenses = useMemo(() => {
    return expensesForMonth
      ...
      .reduce(
        (acc, exp) =>
          acc +
          (exp.installments ? exp.amount / exp.installments.total : exp.amount),
        0
      );
  }, [expensesForMonth]);

  const midMonthSurplus =
    investmentFrequency === 'bi-monthly'
      ? fortnightlyIncome - creditCardExpenses
      : 0;

  const handleFortnightlyIncomeChange = (
  ...
  const handlePercentageChange =
    ...
    (jarId: string, percentage: number) => {
    ...
    <div className='mt-2 space-y-8'>
          ...
          <button
            ...
            className={`w-full text-center font-semibold py-2 px-4 rounded-lg transition-colors duration-200 ${
              investmentFrequency === 'bi-monthly'
                ...
                : 'bg-transparent text-slate-400 hover:bg-dark-700'
            }`}
          >
      ...
      <div className='bg-dark-800 p-6 rounded-xl shadow-lg'>
        <h2 className='text-xl sm:text-2xl font-bold text-slate-100 mb-4'>
          Configurar Ganhos para Investimento
        </h2>
        <div className='space-y-4'>
          {investmentFrequency === 'bi-monthly' && (
            <div>
              <label className='block mb-1 font-semibold text-slate-300'>
                Receita Quinzenal (adiantamento)
              </label>
              <input
                type='text'
                inputMode='decimal'
                value={fortnightlyIncomeInput}
                onChange={handleFortnightlyIncomeChange}
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 items-start'>
              ...
              <div className='flex justify-between items-center text-xl'>
                ...
                <span
                  className={`font-extrabold ${
                    midMonthSurplus >= 0 ? 'text-blue-400' : 'text-yellow-400'
                  }`}
                >
                  {formatCurrency(midMonthSurplus, isCensored)}
                </span>
              </div>
            </div>
            <hr className='border-dark-600 mb-4' />
            <p className='text-slate-400 mb-4 text-center'>
              Distribua o saldo para investir nas suas caixinhas:
            </p>
            <SavingsManager
              jars={jars}
        ...
        <div
          className={`bg-dark-800 p-6 rounded-xl shadow-lg ${
            investmentFrequency === 'monthly' ? 'lg:col-span-2' : ''
          }`}
        >
          <h2 className='text-xl sm:text-2xl font-bold text-slate-100 mb-4'>
            Planejamento Fim de Mês
          </h2>
          <div className='flex justify-between items-center text-xl'>
            <span className='text-slate-200'>Aporte sugerido:</span>
            <span className='font-extrabold text-green-400'>
              {formatCurrency(monthlyPayment, isCensored)}
            </span>
          </div>
          <p className='text-slate-500 mt-2 text-sm'>
            Este é o valor que você configurou como seu pagamento de fim de mês.
          </p>
          <hr className='border-dark-600 my-4' />
          <p className='text-slate-400 mb-4 text-center'>
            Distribua o aporte nas suas caixinhas:
          </p>
          <SavingsManager
            jars={jars}
...
const MonthlyAlert: React.FC<{
  current: number;
  previous: number;
  isCensored: boolean;
}> = ({ current, previous, isCensored }) => {
  if (previous === 0 && current > 0) {
    return (
      <div className='bg-yellow-500/20 text-yellow-300 p-4 rounded-lg mb-6 flex items-center'>
        <TrendingUpIcon /> Primeiro mês com gastos registrados. Mantenha o
        controle!
      </div>
    );
  }
  ...
  if (difference > 0) {
    return (
      <div className='bg-danger/20 text-red-400 p-4 rounded-lg mb-6 flex items-center'>
        <TrendingUpIcon /> Seus gastos aumentaram em{' '}
        {formatCurrency(difference, isCensored)} este mês.
      </div>
    );
  } else if (difference < 0) {
    return (
      <div className='bg-success/20 text-green-400 p-4 rounded-lg mb-6 flex items-center'>
        <TrendingDownIcon /> Ótimo! Seus gastos diminuíram em{' '}
        {formatCurrency(Math.abs(difference), isCensored)} este mês.
      </div>
    );
  } else {
    return (
      <div className='bg-blue-500/20 text-blue-400 p-4 rounded-lg mb-6 flex items-center'>
        <MinusCircleIcon /> Seus gastos se mantiveram estáveis este mês.
      </div>
    );
  }
};
  ...
  const { currentBill, futureDebt, totalDebt } = useMemo(() => {
    ...
    const currentBill = currentBillExpenses.reduce((acc, exp) => {
      const amount = exp.installments
        ? exp.amount / exp.installments.total
        : exp.amount;
      return acc + amount;
    }, 0);

    // Calculate Future Debt from installments
    const installmentExpenses = creditCardExpenses.filter(
      (e) => e.installments
    );
    const futureDebt = installmentExpenses.reduce(
      (totalFutureDebt, expense) => {
        const installmentAmount = expense.amount / expense.installments!.total;
        ...
        const monthsDiff =
          (displayedDate.getFullYear() - firstPaymentDate.getFullYear()) * 12 +
          (displayedDate.getMonth() - firstPaymentDate.getMonth());
        ...
        const remainingInstallments =
          expense.installments!.total - paidInstallments;
      ...
      <div className='space-y-1 mt-2'>
        ...
        <div className='flex justify-between items-baseline'>
          ...
          <span className='font-bold text-lg text-red-400'>
            {formatCurrency(futureDebt, isCensored)}
          </span>
        </div>
        <hr className='border-dark-700 !my-2' />
        <div className='flex justify-between items-baseline'>
          <span className='text-slate-200 font-bold'>Dívida Total</span>
          <span className='font-extrabold text-2xl text-danger'>
            {formatCurrency(totalDebt, isCensored)}
          </span>
        </div>
      </div>
    </div>
...
const ExpenseCategoryChart: React.FC<{
  ...
  isCensored: boolean;
}> = ({ expenses, totalExpenses, isCensored }) => {
  ...
  const categoryData = useMemo(() => {
    ...
    const expensesByCategory = expenses.reduce(
      (acc: Record<string, number>, expense) => {
        ...
        const amount = expense.installments
          ? expense.amount / expense.installments.total
          : expense.amount;
  ...
  return (
    <div className='w-full'>
      <h3 className='text-lg font-bold text-slate-300 mb-4 text-center lg:text-left'>
          ...
          <div className='relative w-40 h-40 mx-auto flex-shrink-0'>
                ...
                return (
                  ...
                  >
                    <path
                      ...
                      style={{
                        transform: isSliceActive ? 'scale(1.05)' : 'scale(1)',
            ...
            {activeSlice ? (
              <div className='absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center'>
                <p className='text-sm text-slate-300 break-words px-2'>
                  {activeSlice}
                </p>
                <p className='font-bold text-lg text-white'>
                  {formatCurrency(
                    categoryData.find((d) => d.name === activeSlice)?.value ||
                      0,
                    isCensored
                  )}
                </p>
              ...
              return (
                <React.Fragment key={slice.name}>
                  <li
                    ...
                    onMouseLeave={() => setActiveSlice(null)}
                    onClick={() =>
                      hasSubcategories && handleToggleCategory(slice.name)
                    }
                  >
                      ...
                      {subcategories.map((sub) => (
                        ...
                        >
                          ...
                          <span className='font-medium'>
                            {formatCurrency(sub.value, isCensored)}
                          </span>
                        </li>
                      ))}
                    </ul>
...
const FinancialChart: React.FC<{
  ...
  isCensored: boolean;
}> = ({ income, expenses, surplus, monthlyExpensesList, isCensored }) => {
  if (income <= 0) {
    return (
      <div className='bg-dark-800 p-6 rounded-xl shadow-lg mt-8 text-center'>
        ...
        <p className='text-slate-400'>
          Adicione uma receita para ver o gráfico de distribuição.
        </p>
      </div>
    );
  }
          ...
          <div className='relative w-48 h-48 sm:w-52 sm:h-52'>
            <svg className='w-full h-full' viewBox='0 0 200 200'>
              <text
                ...
                y='95'
                textAnchor='middle'
                className='fill-current text-slate-400 text-sm'
              >
                Receita Total
              </text>
              <text
                ...
                y='120'
                textAnchor='middle'
                className='fill-current text-slate-100 text-2xl font-bold'
              >
                {formatCurrency(income, isCensored)}
              </text>
              ...
              <circle
                ...
                fill='transparent'
                strokeWidth='20'
                className='text-blue-500/10 stroke-current'
              />
              ...
              {surplus > 0 && (
                <circle
                  ...
                  strokeDashoffset={
                    circumference - (surplusPercentage / 100) * circumference
                  }
                  strokeLinecap='round'
              ...
              <circle
                ...
                strokeDasharray={circumference}
                strokeDashoffset={
                  circumference - (expensesPercentage / 100) * circumference
                }
                strokeLinecap='round'
                transform={`rotate(${
                  (surplus > 0 ? surplusRotation : 0) - 90
                } 100 100)`}
            ...
            <div className='flex items-center'>
              ...
              <div>
                ...
                <p className='font-bold text-base text-red-400'>
                  {formatCurrency(expenses, isCensored)}
                </p>
              </div>
            </div>
            <div className='flex items-center'>
  ...
  return (
    <div className='bg-dark-800 p-6 rounded-xl shadow-lg flex flex-col'>
      <div className='flex justify-between items-center mb-4'>
        ...
        <button
          onClick={onAddIncome}
          className='flex items-center bg-primary hover:bg-secondary text-white font-bold py-2 px-3 sm:px-4 rounded-lg transition-colors'
        >
        ...
        {incomes.length === 0 ? (
          ...
          <ul className='space-y-3'>
            {incomes.map((inc) => (
              <li
                key={inc.id}
                className='bg-dark-700 p-3 rounded-lg flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2'
              >
                <p className='font-semibold self-start'>{inc.description}</p>
                <div className='flex items-center self-end sm:self-center'>
                  <p className='font-bold text-green-400 mr-4'>
                    {formatCurrency(inc.amount, isCensored)}
                  </p>
                  <button
...
const ExpenseFilter: React.FC<ExpenseFilterProps> = ({
  ...
  const availableSubcategories =
    SUBCATEGORIES[categoryFilter as keyof typeof SUBCATEGORIES] || [];
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end'>
        <div>
          ...
          <select
            ...
            className='w-full bg-dark-700 p-2 rounded border border-dark-600 focus:outline-none focus:ring-2 focus:ring-accent text-sm'
          >
        ...
        <div>
          ...
          <select
            ...
            className='w-full bg-dark-700 p-2 rounded border border-dark-600 focus:outline-none focus:ring-2 focus:ring-accent disabled:opacity-50 disabled:cursor-not-allowed text-sm'
          >
            <option value='all'>Todas</option>
        ...
        <button
          ...
          className='w-full bg-dark-600 hover:bg-dark-500 text-slate-200 font-bold py-2 px-4 rounded-lg transition-colors text-sm'
        >
          Limpar Filtros
        </button>
      </div>
...
const ExpenseManager: React.FC<ExpenseManagerProps> = ({
  ...
  const monthYearDisplay = displayedDate.toLocaleString('pt-BR', {
    month: 'long',
    year: 'numeric',
  });
    ...
    <div className='bg-dark-800 p-6 rounded-xl shadow-lg flex flex-col'>
      <div className='flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4'>
        <div className='flex items-center gap-2 self-center sm:self-auto'>
          <h2 className='text-2xl font-bold text-slate-100 hidden sm:block'>
            Despesas
          </h2>
          <button
          ...
          <span className='font-semibold text-lg text-slate-400 capitalize w-36 text-center'>
            {monthYearDisplay}
          </span>
          <button
            onClick={onNextMonth}
        ...
        <button
          ...
          className='flex items-center justify-center bg-primary hover:bg-secondary text-white font-bold py-2 px-3 sm:px-4 rounded-lg transition-colors'
        >
          <PlusIcon className='h-5 w-5' />
          <span className='hidden sm:inline ml-2'>Adicionar</span>
        </button>
      ...
      <div className='flex-grow overflow-y-auto max-h-96 pr-2'>
        {expenses.length === 0 ? (
          <p className='text-slate-400 text-center py-10'>
            Nenhuma despesa encontrada para este mês com os filtros aplicados.
          </p>
        ) : (
          <ul className='space-y-3'>
            {expenses.map((exp) => {
    ...
    <div className='bg-dark-800 p-6 rounded-xl shadow-lg flex flex-col'>
      ...
      {isDistributionDisabled && jars.length > 0 && (
        <div className='bg-yellow-500/20 text-yellow-300 p-3 rounded-lg mb-4 flex items-center gap-2'>
          ...
          <p>
            Saldo insuficiente para distribuir. Os valores não serão aplicados.
          </p>
        </div>
      )}
      ...
      <div className='flex-grow'>
        <div className='mb-4'>
          <div className='w-full bg-dark-600 rounded-full h-4'>
            <div
              className={`rounded-full h-4 text-xs flex items-center justify-center text-white ${
                totalPercentage > 100 ? 'bg-danger' : 'bg-success'
              }`}
          ...
          {totalPercentage < 100 && totalPercentage > 0 && (
            <p className='text-yellow-400 text-sm mt-1'>
              Faltam {100 - totalPercentage}% para distribuir.
            </p>
          )}
        </div>
        {jars.length === 0 ? (
          <p className='text-slate-400 text-center py-10'>
            Crie caixinhas para guardar o dinheiro que sobra.
          </p>
        ) : (
              ...
              return (
                <li key={jar.id} className='bg-dark-700 p-3 rounded-lg'>
                  <div className='flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center'>
                    <span className='font-semibold self-start sm:self-center'>
                      {jar.name}
                    </span>
                  ...
                  <p className='text-accent font-bold mt-1 text-right sm:text-left'>
                    {formatCurrency((surplus * percentage) / 100, isCensored)}
                  </p>
                </li>
              );
            })}
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <button
          type='submit'
          className='w-full bg-primary hover:bg-secondary text-white font-bold py-2 px-4 rounded-lg transition-colors'
        >
...
const ExpenseModal: React.FC<{
  ...
  const resetForm = useCallback(() => {
    ...
    setSubcategory(
      SUBCATEGORIES[EXPENSE_CATEGORIES[0] as keyof typeof SUBCATEGORIES]?.[0] ||
        ''
    );
  ...
  useEffect(() => {
    if (!isEditing && category) {
      setSubcategory(
        SUBCATEGORIES[category as keyof typeof SUBCATEGORIES]?.[0] || ''
      );
    }
  }, [category, isEditing]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isInstallment) {
      ...
      if (!isEditing && currentInstallmentNum > 1) {
        currentPaymentDate.setMonth(
          currentPaymentDate.getMonth() - (currentInstallmentNum - 1)
        );
      }
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center'>
            <input
              ...
              className='mr-2 h-4 w-4 rounded accent-accent'
            />
          ...
          <div className='flex items-center'>
            <input
              ...
              className='mr-2 h-4 w-4 rounded accent-accent disabled:opacity-50'
            />
            <label
              htmlFor='isRecurring'
              className={isInstallment ? 'text-slate-500' : ''}
            >
              É recorrente?
            </label>
          </div>
